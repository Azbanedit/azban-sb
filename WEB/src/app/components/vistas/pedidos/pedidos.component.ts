...
export class PedidosComponent implements OnInit {
  ...
  openViewAllActive : boolean = false;
  ...
  viewOrder(order : any)
  {
    this.imagesList = new Array<string>();
    this.mainImage = order.id + '/' + order.id + '-' + 1 + '.jpg'

    for(let i = 1; i <= order.numImages; i++)
    {
      this.imagesList.push(order.id + '/' + order.id + '-' + i + '.jpg');
    }
  ...
  viewAllDetails(){
    this.openViewAllActive = !this.openViewAllActive;

    for(let i = 0; i < this.openView.length; i++)
    {
      this.openView[i] = this.openViewAllActive;
    }
  }
  ...
  payIcon(orderId: number){
    ...
    this.paymentSelectedId = orderId
  }

  saveChangePay(){
    this.showConfirmation = false;
    
    this.serviceClient.payOrder(this.paymentSelectedId).subscribe(()=>{

      for(let i = 0; i < this.orderData.length; i++)
      {
        if (this.orderData[i].id == this.paymentSelectedId)
        {
          this.orderData[i].payed = true;
          break;
        }
      }
  ...
  changeState(index : number, orderId : number){
    ...
    this.serviceClient.getChangeState(orderId, newStateId).subscribe((resp: any) =>{
      this.orderData.find(order => order.id === orderId).stateId = newStateId;
    });
  }

  getColorState(stateId : number)
  {
    switch (stateId)
    {
        case 2 :
...
// Produccion = 2, Creados = 1, Listo PE = 3, Todos = 0
  ...
  checkClothes(idProduct : number, checkProductBaseExist: boolean, orderId : number){

    if (!checkProductBaseExist)
    {
      let option = document.getElementById('control-producto-op1-' + idProduct);
      option?.classList.add('back-green');
      let option2 = document.getElementById('control-producto-op2-' + idProduct);
      option2?.classList.remove('back-red');
    }
    else 
    {
      let option = document.getElementById('control-producto-op1-' + idProduct);
      option?.classList.remove('back-green');
      let option2 = document.getElementById('control-producto-op2-' + idProduct);
      option2?.classList.add('back-red');
    }

    this.serviceClient.checkProductExist(idProduct , checkProductBaseExist).subscribe((resp:any)=>{
      for(let i = 0; i < this.orderData.length; i++){

        if (this.orderData[i].id == orderId)
        {
          for(let j = 0; j < this.orderData[i].products.length; j++)
          {
            if(this.orderData[i].products[j].id == idProduct)
            {
              this.orderData[i].products[j].readyProductBase = checkProductBaseExist;
              break;
            }
          }
