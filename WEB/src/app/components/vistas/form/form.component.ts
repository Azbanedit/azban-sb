...
export class FormComponent implements OnInit {
  imagesList : Array<Base64Image> = new Array<Base64Image>();
  ...
  captureFile(event: any) {
    ...
    let saveImage = event.target.files[0];
  //  console.log(event.target.files);
    this.images.push(saveImage);
  }

  deleteSelectImage(img : Base64Image)
  {
    this.imagesList?.splice(this.imagesList.indexOf(img), 1);
  }

  onLoadImagesEvent(files : Array<Base64Image>)
  {
    this.imagesList = this.imagesList?.concat(files);
